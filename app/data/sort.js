...
// Canonical status rank; unknown statuses sort after known ones.
const STATUS_RANK = { open: 0, in_progress: 1, closed: 2 };
...
function columnKey(issue, column) {
  switch (column) {
    case 'status': {
      const rank = STATUS_RANK[/** @type {keyof STATUS_RANK} */ (issue.status)];
